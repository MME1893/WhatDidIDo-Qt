...
extern const QString sTr(const QString msg);
...
{
    ui->setupUi(this);
    ui->widget_mdpt_top->setLayoutDirection(Qt::LeftToRight);
...
void QPersianDateTimeWidget::setPDateTime(QPersianDateTime dateTime)
{
    ...
    ui->btn_timeAP->setText(sTr(selectedPDateTime->getGDateTime().toString("ap").toUpper()));
    ui->btn_timeAP->setProperty("val",dateTime.toString("ap"));
}
...
{
    qint8 hour  = ui->line_hour->text().toInt() + ( 12*((ui->btn_timeAP->property("val").toString() == "pm" )?1:0) );
...
void QPersianDateTimeWidget::on_btn_toggleDateTimeWidget_clicked()
{
    ...
    {
        ui->widget_mdpt_top->setVisible(true);
        ui->btn_toggleDateTimeWidget->setIcon(QIcon(":/icon/res/images/time.png"));
    }
    else
    {
        ui->widget_mdpt_top->setVisible(false);
...
{
    if( ui->btn_timeAP->property("val").toString() == "am" ) {
        ui->btn_timeAP->setText(sTr("PM"));
        ui->btn_timeAP->setProperty("val","pm");
    }
    else {
        ui->btn_timeAP->setText(sTr("AM"));
        ui->btn_timeAP->setProperty("val","am");
    }
    updateTime();
}
